...
public class OrderController {
    ...
    public ResponseEntity<?> filter(@RequestParam(required = false) String startDate,
                                    @RequestParam(required = false) String endDate){

        log.debug("Filtering Orders");
        return new ResponseEntity<>(orderService.filter(startDate, endDate), HttpStatus.OK);
    }
